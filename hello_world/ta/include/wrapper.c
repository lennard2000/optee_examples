...
psa_status_t psa_cipher_update(TEE_OperationHandle
    ...
    TEE_Result result = TEE_CipherUpdate(sessionctx->op_handle, input, input_size, output, (uint32_t *) &output_size);
    if (result != TEE_SUCCESS) {
        IMSG("Error updating cipher");
...
psa_status_t psa_cipher_finish(TEE_OperationHandle
    ...
    uint8_t *emptyInput;
    TEE_Result res = TEE_CipherDoFinal(sessionctx->op_handle, emptyInput, 0, output, (uint32_t *) &output_size);
    if (res != TEE_SUCCESS) {
